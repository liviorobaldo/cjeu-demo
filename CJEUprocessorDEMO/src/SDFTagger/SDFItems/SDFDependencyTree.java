...
//An SDFDependencyTree is a collection of SDFHead(s) defining a DependencyTree (we check that in the constructor)
public class SDFDependencyTree 
{
    ...
    public SDFDependencyTree(SDFHead[] heads)throws Exception
    {
        ...
        for(int i=0;i<heads.length;i++)
        {
            ...
            {
                    //checking the head with the next recursive procedure
                checkSDFHead(heads[i], heads, safeSDFHead);
            }
        }
    }
    ...
    public SDFHead[] getHeads()
    {
        ...
        //UTILITY
        //Recursive (private!) procedure to check whether the heads defines a dependency tree.
        //We check: (1) the governor belongs to heads; (2) there is not a loop, i.e., we cannot find the same node on a chain of governors.
    private boolean checkSDFHead(SDFHead newSDFHead, SDFHead[] heads, Hashtable<SDFHead,String> safeSDFHead)throws Exception
    {
        if(safeSDFHead.get(newSDFHead)!=null)return true;//if it is already in the hashtable, the SDFHead is "safe": it has been previously checked already.
        
            //if the governor is null, this is the root: it is fine
        if(newSDFHead.Governor==null)return true;
        
            //If not, only if its governor is safe, the head is safe. 
            //The governor is safe either if it belongs to safeSDFHead or if it belongs to heads and its governor is (recursively) safe
